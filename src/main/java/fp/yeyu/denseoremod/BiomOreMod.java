...
public class BiomOreMod implements DedicatedServerModInitializer, ModInitializer {
	...
	private static void registerBiomoreHereCommand(CommandDispatcher<ServerCommandSource> dispatcher, boolean isDedicated) {
		...
		dispatcher.register(
				CommandManager.literal("biomore").executes(BiomOreMod::executeBiomoreCommand)
		);
	}

	private static int executeBiomoreCommand(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
		final ServerPlayerEntity player = context.getSource().getPlayer();
		final Biome biome = player.getServerWorld().getBiome(player.getBlockPos());
		context.getSource().sendFeedback(new LiteralText("Obtainable ore: ").formatted(Formatting.GOLD), false);
		final Biome fallbackBiomeKey = BiomOreFeatures.FALLBACK_BIOME.getOrDefault(biome.getCategory(), Biomes.PLAINS);
		final String fallback = BiomOreFeatures.BIOME_CONTAINS.getOrDefault(fallbackBiomeKey, "<err>");
		context.getSource().sendFeedback(new LiteralText("  " + BiomOreFeatures.BIOME_CONTAINS.getOrDefault(biome, fallback)), false);
		return 1;
	}
